...
local function instagramUser(msg, query)
	...
	if res ~= 200 then
		return "📛لا يوحد اتصال ❌"
    end
	local jdat = json:decode(jstr)
	if #jdat.data == 0 then
		send_msg(receiver,"#عذرا💔\n📛لم يتم العثور ع اليوز او ربما الصفحه غير موجوده ❌او خاصه🔒",ok_cb,false)
	end
	...
	if user.data.bio ~= '' then
		text = text.."اليوز💟 : "..user.data.username:upper().."\n\n"
	else
		text = text.."اليوز💟 : "..user.data.username:upper().."\n"
	end
	if user.data.bio ~= '' then
	...
	if user.data.full_name ~= '' then
		text = text.."الاسمⓂ️: "..user.data.full_name.."\n"
	end
...
local function instagramMedia(msg, query)
	...
	if jdat.data.caption then
	      ...
	      text = text.."اليوز💟 : "..data.from.username:upper().."\n\n"
		  text = text..data.from.full_name.."\n\n"
		  text = text..data.text.."\n\n"
		  text = text.."عدد لايك ❤️: "..jdat.data.likes.count.."\n"
    else
	      text = text.."اليوز💟 : "..jdat.data.user.username:upper().."\n"
		  text = text.."الاسمⓂ️: "..jdat.data.user.full_name.."\n"
		  text = text.."عدد لايك ❤️: "..jdat.data.likes.count.."\n"
	end
	text = text.."\n@Th3_BOOS\n❣ #قناة_البوت : @dev_Th3_BOOS"
	send_msg(receiver,text,ok_cb,false)
