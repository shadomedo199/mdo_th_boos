...
local function kick_ban_res(extra, success, result)
      ...
      elseif get_cmd == 'حظر' then
        ...
        send_large_msg(receiver, '💠العضو @'..member..' ['..member_id..'] تم✅حظره🚷✔️')
		ban_user(member_id, chat_id)
      elseif get_cmd == 'الغاء الحظر' then
        send_large_msg(receiver, '💠العضو @'..member..' ['..member_id..'] تم✅ فك 🔓الحظر 🚷عنه✔️')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return '💠العضو '..user_id..' تم✅فك حظره 📛عنه✔️'
      elseif get_cmd == 'حظر عام' then
        send_large_msg(receiver, '💠العضو @'..member..' ['..member_id..'] تم✅حظره🚷عامⓂ️✔️ ')
		banall_user(member_id)
      elseif get_cmd == 'الغاء العام' then
        send_large_msg(receiver, '💠العضو @'..member..' ['..member_id..'] تم✅ فك 🔓العام 🚷عنه✔️')
...
local support_id = msg.from.id
 if matches[1]:lower() == 'ايدي' and msg.to.type == "chat" or msg.to.type == "user" then
  ...
  if matches[1]:lower() == "قائمه المحظورين" then -- قائمه المحظورين :
    local chat_id = msg.to.id
  ...
  if matches[1]:lower() == 'حظر' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
		...
		send_large_msg(receiver, '💠العضو ['..matches[2]..'] تم✅ حظره 📛✔️')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'حظر',
  ...
  if matches[1]:lower() == 'الغاء الحظر' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(targetuser, '^%d+$') then
        	...
        	return '💠العضو ['..user_id..'] تم فك🚷 الحظر عنه✔️'
      else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'الغاء الحظر',
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return '💠العضو ['..user_id..' ] تم✅ حظره 📛عام ✔️'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'حظر عام',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return '💠العضو ['..user_id..' ]تم فك 🚷العامه 🌀عنه✔️'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'الغاء العام',
